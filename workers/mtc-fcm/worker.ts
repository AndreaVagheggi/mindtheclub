...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    //
    // toUserId is the only identity this relay can see: everything else is
    // sealed to the recipient (see docs/arch/identity.md, sealed sender). That
    // makes it the only possible key, and it is a good one: a runaway loop is
    // one phone hammering one peer, which is exactly what this catches.
    //
    // Unlike the counter this replaced, the limit applies to whoever is
    // misbehaving and not to every user in the world, it is enforced in the
    // Cloudflare location serving the request with no shared state, and it
    // therefore has no ceiling of its own. A per-key limit does not care
    // whether there are four users or nine billion.
    //
    // 120 per 60 s is about two messages a second to a single recipient,
    // sustained. No real conversation reaches it; a loop does immediately.
