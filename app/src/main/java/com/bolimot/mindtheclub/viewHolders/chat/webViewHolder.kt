...
class WebViewHolder(itemView: View, private val listener: MessagesAdapter.OnItemClickListener) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(message: Message?, displayDateHeader: Boolean, isSelected: Boolean, subType: String?) {
        try {
            message?.let {
                val isNewMessage = it.messageId != lastBoundMessageId
                lastBoundMessageId = it.messageId
                needsScrollAdjust = isNewMessage

                if (isNewMessage) {
                    try {
                        val imageToLoad = it.replyId ?: R.drawable.image
