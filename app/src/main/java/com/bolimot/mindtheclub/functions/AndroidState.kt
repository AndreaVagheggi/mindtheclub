 ...
 *
 * Every symptom chased through August ended up here and could not be told apart from the logs:
 * retries 13 and 19 minutes apart instead of 15 and 30 seconds, wake-ups accepted by Google and
 * never delivered, transfers frozen at 82%. Doze, an app standby bucket that had drifted down,
 * and an exhausted expedited quota all look identical from inside the app, and all three are
 * decided outside it. Indovinare quale fosse in gioco cost days.
 *
...
object AndroidState {
     ...
     *
     * bucket matters most and is the least visible: it drives BOTH how fast a high priority FCM
     * is delivered and how much expedited quota is left, so a phone quietly demoted to rare or
     * restricted starts failing in two ways at once. exempt is the battery optimisation setting
     * the user can change; restricted is the harder "Restrict background usage" switch; idle
     * says whether the device is in doze at this instant.
     */
    ...
    /**
     * The standby bucket in words. Below API 28 the concept does not exist, and from API 30 the
     * system may also report RESTRICTED, the harshest one.
     */
    ...
    /**
     * Why the system stopped a worker. The answer that was missing every time a dispatch died
     * mid transfer: until now a stopped worker and a failed connection looked the same in the
     * log.
     *
     * QUOTA is the one to watch for, it means the expedited allowance ran out and the work was
     * demoted; APP_STANDBY and DEVICE_IDLE mean the bucket or doze took it.
     */
