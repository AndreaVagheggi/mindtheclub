...
/**
 * Runs the assembly of a fully received multi chunk message (chunks -> file -> Message row) under
 * WorkManager protection, the same doze guarantee the send path already has
 * (submitSendMessageWorker / SendFcmWorker).
 *
 * Without this, assembly ran on a bare coroutine: when the last chunk arrived and DataSyncService
 * stopped, doze froze the process mid assembly and the completed message sat invisible in the
 * Inbox until the next manual wake-up.
 *
 * The work is awaited inside doWork(), so WorkManager keeps the process alive until assembly
 * finishes, and re-runs it after a process death.
 */
class AssembleMessageWorker(
    ...
    companion object {
        ...
        fun enqueue(context: Context, messageId: String) {
            // Deliberately NOT expedited: setExpedited() requires a getForegroundInfo()
            // implementation on API < 31 (crash otherwise), and a plain request already starts
            // immediately while the process is alive and re-runs after doze or process death.
            val request = OneTimeWorkRequestBuilder<AssembleMessageWorker>()
                .setInputData(workDataOf(KEY_MESSAGE_ID to messageId))
                .build()

            // Unique per message plus KEEP: repeated completion signals (inbox checks, the
